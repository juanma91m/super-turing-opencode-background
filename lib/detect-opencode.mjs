...
function detectInstalledBinary() {
  ...
  if (
    ...
    return {
      method: "curl-binary",
      execPath,
      installRoot: path.resolve(path.dirname(execPath), ".."),
      version,
    };
  }

  return {
    method: "binary-or-global",
    execPath,
    installRoot: path.resolve(path.dirname(execPath), ".."),
    version,
  };
...
export async function detectOpencodeTarget({
  ...
  const explicitInstallRoot = expandHome(
    installRoot || process.env.OPENCODE_INSTALL_ROOT,
  );
  if (explicitInstallRoot) {
    const normalized = path.resolve(explicitInstallRoot);
    const execPath = path.join(normalized, "bin", "opencode");
    if (!requireExecutable(execPath)) {
      return {
        supported: false,
        method: "unknown",
        installRoot: normalized,
        message: `El install root explícito no contiene un binario usable en ${execPath}`,
      };
    }
    const versionProc = spawnSync(execPath, ["--version"], {
      encoding: "utf8",
    });
    const version =
      versionProc.status === 0 ? versionProc.stdout.trim() : undefined;
    return {
      ...
      installRoot: normalized,
      version,
      message: `Se detectó una instalación local explícita en ${normalized}`,
    };
  }
