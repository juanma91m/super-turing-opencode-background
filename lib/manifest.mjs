...
export async function loadManifest() {
  ...
  const assetManifests = Array.isArray(addon.assets) ? addon.assets : [];
  const assets = assetManifests.map((asset) => ({
    manifest: asset,
    sourcePath: resolveFromAddon(...asset.source.split("/")),
    installedPath: resolveInstalledAssetPath(asset),
  }));
