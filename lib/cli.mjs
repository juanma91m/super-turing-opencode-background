export function parseArgs(argv = process.argv.slice(2)) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (value === "--checkout-root") {
      const next = argv[index + 1];
      if (!next || next.startsWith("--")) {
        options.errors.push("--checkout-root requiere una ruta");
        continue;
      }
    ...
    if (value === "--bun-path") {
      const next = argv[index + 1];
      if (!next || next.startsWith("--")) {
        options.errors.push("--bun-path requiere una ruta");
        continue;
      }
      options.bunPath = next;
      index += 1;
      continue;
    }
    if (value === "--install-root") {
      const next = argv[index + 1];
      if (!next || next.startsWith("--")) {
        options.errors.push("--install-root requiere una ruta");
        continue;
      }
...
export function printHelp(commandName) {
  console.log(
    `Usage: node ./scripts/${commandName}.mjs [--opencode-root /path/to/opencode] [--checkout-root /path/to/opencode] [--install-root /path/to/install] [--bun-path /path/to/bun] [--dry-run] [--json]`,
  );
}
